...
function tiktokMeetsCriteria(info) {
  const now = Date.now();
  const FIVE_MINUTES = 5 * 60 * 1000;

  if (info.followers < 10000) return false;
  if (now - info.createTime > FIVE_MINUTES) return false;
...
async function highlightYouTube(node, youtubeLink) {
  ...
  chrome.runtime.sendMessage(
    ...
    (response) => {
      ...
      // // Example criteria: uploaded in the last 5 minutes
      // const FIVE_MINUTES = 1000 * 60 * 60 * 1000;
      // if (Date.now() - uploadTime <= FIVE_MINUTES) {
        // node.style.border = "2px solid gold";
      // }
    }
  );
...
async function highlightTikTok(node, tiktokLink) {
  chrome.runtime.sendMessage(
    ...
    (response) => {
      ...
      // if (tiktokMeetsCriteria({ createTime, followers })) {
...
async function highlightInstagram(node, instagramLink) {
  ...
  chrome.runtime.sendMessage(
    ...
    (response) => {
      ...
      // const thirtyMinutes = 1000 * 60 * 60 * 1000;

      // if (Date.now() - createTime <= thirtyMinutes) {
